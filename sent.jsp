...
<%
	...
	String to=(String)session.getAttribute("uname");
		Date d = new Date();
    try
	{
		...
		PreparedStatement ps=con.prepareStatement("select * from sent where from1=? order by messid desc");
		ps.setString(1,to);
		ResultSet rs=ps.executeQuery();
%>
<form method="post" action="deleterecord1.jsp"> 
<body id="yadcf_example">
    <div id="container">
     
      <table cellpadding="0" cellspacing="0" border="0" class="display" id="example" width="924">
	        <thead>
...
%>
			...
			<td><input type="checkbox" name="cbox" value="<%=rs.getString(1)%>"><font face="Century Schoolbook" color="blue"><b><%=rs.getString(2) %></td>
			<td> <a href="readsent1.jsp?mess=<%=rs.getString(1)%>" style="text-decoration:none"><%=rs.getString(3) %></a></td>
			<td><%=str %></td>
			<td><%=dd %></td>
			</tr>
...
%>
			...
			<td><input type="checkbox" name="cbox" value="<%=rs.getString(1)%>"><%=rs.getString(2) %></td>
			<td><a href="readsent1.jsp?mess=<%=rs.getString(1)%>" ><%=rs.getString(3) %></a></td>
			<td><%=str %></td>
			<td><%=dd %></td>
